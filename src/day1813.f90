...
contains
    function mine_new(file) result(new)
        ...
        print *, 'board size: ',shape(new%board)
    end function


    function cart_new(id, pos, ch) result(new)
        type(cart_t) :: new
    ...
    character(len=1) function track_under_cart(ch) result(res)
        character(len=1), intent(in) :: ch
        select case(ch)
    ...
    subroutine mine_movecarts(this)
        class(mine_t), intent(inout) :: this

        integer :: i, collide_with
        call sort_carts(this%carts)
        do i=1,size(this%carts)
            call this%carts(i)%move(this%board)
            print *, this%carts(i)
            collide_with=this%carts(i)%collide(this%carts)
            if (collide_with /= 0) then
                print *, 'collision at:', this%carts(i)%pos(1)-1, this%carts(i)%pos(2)-1
                print *, 'collision with ', this%carts(collide_with)%id
                stop
            end if
    ...
    integer function cart_collide(this, others) result(collision_with)
    ...
    elemental logical function cart_greater_than(a, b) result(gt)
        ...
        ! greater come last (bottom row come last)
        if (a%pos(2)>b%pos(2)) then
            gt = .true.
        else if (a%pos(2)<b%pos(2)) then
            gt = .false.
        else
            ! greater come last (right comes last)
